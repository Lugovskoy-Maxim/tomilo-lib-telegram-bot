...
const { API_BASE_URL, API_TOKEN, BOT_API_SECRET } = require('../config');
...
async function getLinkedUser(telegramUserId) {
    const response = await botApiClient.get(`/telegram/bot/user/${telegramUserId}`);
    return unwrap(response);
}

async function linkAccount(code, telegramUserId, chatId, username) {
    const response = await botApiClient.post('/telegram/bot/link', {
...
async function getBookmarks(telegramUserId) {
    const response = await botApiClient.get(`/telegram/bot/bookmarks/${telegramUserId}`);
    return unwrap(response);
}
